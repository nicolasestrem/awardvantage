...
# Get all candidates as a map: name -> post_id
declare -A candidate_map

while IFS=$'\t' read -r id title; do
    if [[ "$id" != "ID" ]]; then
        candidate_map["$title"]="$id"
    fi
done < <(docker exec awardvantage_wpcli wp post list --post_type=mt_candidate --fields=ID,post_title --format=tsv --allow-root 2>/dev/null)
...
while IFS=',' read -r name linkedin_url photo_filename; do
    ...
    # Get post ID from map
    post_id="${candidate_map[$name]}"
